...
public class MoveableRobot extends Robot {
    ...
    private static final double TURNING_SPEED = 120; //turning speed
    ...
    public void moveForward() {
        setXPos(getXPos() + (getSpeed() * Math.cos(Math.toRadians(getDirection()))));
        setYPos(getYPos() + (getSpeed() * Math.sin(Math.toRadians(getDirection()))));
        arena.checkBounds(this); //checks if the robot is within bounds
    }
